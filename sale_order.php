...
if(!empty($_SESSION['cart'])){
	...
	foreach($_SESSION['cart'] as $key => $qty){
		$id = str_replace('id','',$key); //id string ကို null ပြောင်း။id123...
	...
	$result = $stmt->execute(
		array(':user_id'=>$userId,':total'=>$total,':odate'=>date('Y-m_d H:i:s'))
	);


	if($result){

		$saleOrderId = $pdo->lastInsertId(); //php code
		...
		foreach($_SESSION['cart'] as $key => $qty){
			...
			unset($_SESSION['cart']);//session ပြန်ဖျက်ရန်။
